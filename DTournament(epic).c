...
void BuatTurnamen() {
    ...
    printf("Biaya Pendaftaran (Rp): ");
    scanf("%d", &tourBaru.biayaDaftar); getchar();
...
void MenuUser() {
    ...
    while(1) {
        ...
        printf("3. Daftar Tournament\n");
        printf("4. Lihat List Tournament\n");
        printf("5. Sortir Tournament\n");
        printf("6. Lihat List Tim\n");
...
void TopUpSaldo() {
    ...
    userAktif.saldo += nominal;

    fuser = fopen("users.dat", "rb");
    struct Pengguna users[100];
    int count = 0;

    while(fread(&users[count], sizeof(struct Pengguna), 1, fuser) == 1) {
        if(strcmp(users[count].username, userAktif.username) == 0) {
            users[count].saldo = userAktif.saldo;
        }
        count++;
    }
    fclose(fuser);
    ...
    fwrite(users, sizeof(struct Pengguna), count, fuser);
    fclose(fuser);

    printf("\nTop up berhasil! Saldo baru: Rp %d\n", userAktif.saldo);
    pause();
}
...
void DaftarTurnamen() {
    ...
    while(fread(&temp, sizeof(struct Turnamen), 1, fturnamen) == 1) {
        if(strcmp(temp.status, "Upcoming") == 0) {
            printf("%-5d %-30s %-20s Rp %-12d %d/%d\n",
                   ...
                   temp.biayaDaftar, temp.timTerdaftar, temp.kuotaTim);
            availableCount++;
        }
    }
    fclose(fturnamen);
    ...
    while(fread(&temp, sizeof(struct Turnamen), 1, fturnamen) == 1) {
        if(temp.id == idTournament) {
            found = 1;
            if(userAktif.saldo < temp.biayaDaftar) {
                printf("\nSaldo tidak cukup! Biaya: Rp %d, Saldo Anda: Rp %d\n",
                       temp.biayaDaftar, userAktif.saldo);
                fclose(fturnamen);
                pause();
                return;
            }
    ...
    userAktif.saldo -= temp.biayaDaftar;
    userAktif.isKetuaTim = 1;
    ...
    while(fread(&users[userCount], sizeof(struct Pengguna), 1, fuser) == 1) {
        if(strcmp(users[userCount].username, userAktif.username) == 0) {
            users[userCount].saldo = userAktif.saldo;
...
void LihatListTim() {
    ...
    clearScreen();
    printf("====================================\n");
    printf("=      LIHAT DAFTAR TIM PESERTA    =\n");
    printf("====================================\n\n");
    printf("Masukkan ID Tournament: ");
    scanf("%d", &idTournament); getchar();
    ...
    int found = 0;
    while(fread(&temp, sizeof(struct Tim), 1, ftim) == 1) {
        if(temp.idTurnamen == idTournament) {
            printf("%-20s %-15s %-10s\n",
                   temp.namaTim,
                   temp.idPemain[0],
    ...
    if(!found) {
        printf("Tidak ada tim yang ditemukan untuk ID Tournament %d.\n", idTournament);
    }
...
void TambahKomentar() {
    int idTournament;
    struct Komentar komentarBaru;

    clearScreen();
    printf("====================================\n");
    printf("=      TAMBAH KOMENTAR             =\n");
    printf("====================================\n\n");
    ...
    printf("\nMasukkan ID Tournament yang ingin dikomentari: ");
    scanf("%d", &idTournament); getchar();
    ...
    if(found) {
        komentarBaru.idTurnamen = idTournament;
        strcpy(komentarBaru.username, userAktif.username);

        printf("Isi Komentar: ");
        getStringInput(komentarBaru.isiKomentar, sizeof(komentarBaru.isiKomentar));

        fkomentar = fopen("comments.dat", "ab");
        if(fkomentar != NULL) {
            fwrite(&komentarBaru, sizeof(struct Komentar), 1, fkomentar);
            fclose(fkomentar);
